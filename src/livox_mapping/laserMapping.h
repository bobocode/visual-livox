...
Eigen::Matrix4d laser_to_init;

LaserMapping():nh("laserMapping"),filter_parameter_corner(0.1),filter_parameter_surf(0.2)
{
...
init()
{
 ...
 pubLaserCloudSurround_corner = nh.advertise<sensor_msgs::PointCloud2>("/laser_cloud_surround_corner", 100);
...
void cloudIn(pcl::PointCloud<PointType>::Ptr &laserCloudIn, pcl::PointCloud<PointType>::Ptr &laserCloudSharp,  
      pcl::PointCloud<PointType>::Ptr &laserCloudFlat)
{
 laserCloudCornerLast->clear();
 laserCloudSurfLast->clear();
 laserCloudFullRes->clear();

 laserCloudFullRes = laserCloudIn;
 laserCloudCornerLast = laserCloudSharp;
 laserCloudSurfLast = laserCloudFlat;
...
odomAftMapped.child_frame_id = "/aft_mapped";
 ...
 Eigen::Quaterniond tmp_q(odomAftMapped.pose.pose.orientation.w,odomAftMapped.pose.pose.orientation.x
                          odomAftMapped.pose.pose.orientation.y,odomAftMapped.pose.pose.orientation.z);
 Eigen::Vector3d tmp_p(odomAftMapped.pose.pose.position.x,
                                     odomAftMapped.pose.pose.position.y,
                                     odomAftMapped.pose.pose.position.z);

 laser_to_init.setIdentity();
 laser_to_init.block<3,3>(0,0) = tmp_q.toRotationMatrix();
 laser_to_init.block<3,1>(0,3) = tmp_p;
}
 ...
 if (newLaserCloudCornerLast && newLaserCloudSurfLast && newLaserCloudFullRes &&
  ...
  fabs(timeLaserCloudFullRes - timeLaserCloudCornerLast) < 0.005)
  {
   ...
   while (centerCubeI < 3) {
    ...
    std::cout<<"DEBUG MAPPING laserCloudCornerLast_down : "<<laserCloudCornerLast_down->points.size()<<" laserCloudSurfLast_down : " <<laserCloudSurfLast_down->points.size()<<std::endl;
    std::cout<<"DEBUG MAPPING laserCloudCornerLast : "<<laserCloudCornerLast->points.size()<<" laserCloudSurfLast : " <<laserCloudSurfLast->points.size()<<std::endl;
    std::cout<<"DEBUG MAPPING laserCloudCornerFromMapNum : "<<laserCloudCornerFromMapNum<<" laserCloudSurfFromMapNum : " <<laserCloudSurfFromMapNum<<std::endl;
       ...
       if (pointSearchSqDis[7] < 5.0) 
       {
        ...
        for (int j = 0; j < 8; j++) {
         if (fabs(pa * laserCloudSurfFromMap->points[pointSearchInd[j]].x +
                          pb * laserCloudSurfFromMap->points[pointSearchInd[j]].y + pc * laserCloudSurfFromMap->points[pointSearchInd[j]].z + pd) > 0.2) 
         {
          planeValid = false;
          break;
         }
...
private:
 ...
 ros::NodeHandle nh;
 ros::Publisher pubLaserCloudSurround, pubLaserCloudSurround_corner,pubLaserCloudFullRes,pubOdomAftMapped;
